...
shapes_names= "j1", "tetrahedron", "cube", "octahedron", "j15", "j16", "j86", "hexagonal_antiprism", "j8", "j10", "j49", "j50", "j87", "j88", "j89", "j90", "j12", "j14", "j17", "j51", "j84", "j13"
...
if(__name__ == "__main__"):
    for tessname,tessnet in nets.items():
        p=len(tessnet)
        # print(tessname)
        for facedict in tessnet.values():
            # print(facedict)
            for i in range(len(facedict)):
                facedict[i]= (facedict[i]%p, int((facedict[i]-facedict[i]%p)//p))
    ...
    import pprint
    filename = "TessellationPolyhedronAndTilings.py"
    output = "tessellation_polyhedrons = {\n " + pprint.pformat(polys,indent=4)[1:-1]+"\n}\n"
    output += "\nnet_tessellations = {\n " + pprint.pformat(nets,indent=4)[1:-1]+"\n}\n"
    output += "\n"
    with open(filename,"w") as f:
        f.write(output)
    print("Ouptut",filename,len(polys),"polyhedron nets and",len(nets),"net tessellations")