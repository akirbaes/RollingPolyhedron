...
def area_explore(tiling, tilename, net, polyname, mapping,sp1,sp2, startcase=0, startface=0, startorientation=0, ):
    ...
    RECORD = (polyname,tilename)==("j1",'1u06 (3^2x4x3x4)')
    RECORD = polyname=="j1"
    RECORD = False
    if RECORD:
        pyr = pygame.image.load(".video_output/_j1_scale.png")
    ...
    while visits and (TIMERSTART+TIME_LIMIT>time() and TIME_LIMIT>0) or (RECORD and (counter+1)%(60*5)!=0):
        ...
        visited.add((closest,face,orientation))
        ...
        for index, nextcellid in enumerate(tiling[case]):
            ...
            nextcenter = tuple(centerpoint(nextface_stub))

            # possible_directions.append((nextface, nextcell, nextorientation, pa, pb))

            all_distances = [(distance(nextcenter, other), other) for other in mapping]
            ndist, nclosest = min(all_distances)
            if(ndist<3 and (nclosest,nextface,nextorientation) not in visited):
                possible_directions.append((nextface, nextcell, nextorientation, pa, pb))
                # possible_directions.append((nextface, nextcell, nextorientation, pa, pb))
                # possible_directions.append((nextface, nextcell, nextorientation, pa, pb))
                # possible_directions.append((nextface, nextcell, nextorientation, pa, pb))
        ...
        if not(possible_directions):
            visits.pop()
            visitcount = len(tuple(1 for place,ff,oo in visited if place == closest))
            drawtemp(cface, (32+128/visitcount,32+128/visitcount,32+128/visitcount))
            screen.blit(outlines, (0, 0))
            continue
        ...
        refresh()
        if(RECORD):
            screen.blit(pyr,(0,25))
            pygame.image.save(screen,".video_output/"+str(counter).zfill(4)+".png")
            counter+=1
