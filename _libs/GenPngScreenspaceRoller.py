...
def draw_tiling(spa, spb, surface, startcell, startorientation, tiling: dict, iterlevel=0, itercolors=[(0, 0, 0)],
                visitedcoords=[]):
    draw_numbers = False
    draw_cursor = False
    if (iterlevel < 0):
    ...
    while (insidecells):
        ...
        for index in range(len(tiling[cell])):
            ...
            try:
                nextcell_shift = tiling[nextcell].index((cell, -nextp))
            except:
                nextcell_shift = tiling[nextcell].index((cell, nextp))
...
def map_screenspace(tiling, startcell, area, p1, p2, precision, limit=False):
    ...
    while (visits):
        ...
        # cface is already starting at indice 0 thanks to the line that starts with nextface_stub
...
def draw_answer(filename, tilingname, polyname, visits, grid, polyhedron, p1, p2, startface, startorientation, w, h,
    ...
    if (svg):
        ...
        xx, yy, XX, YY = draw_polynet(s, s, polyhedron, startface, startorientation, p1, p2, tilingname, polyname,
                                      unusedfaces, svg=dwg)
        dwg.save()
    else:
        ...
        xx, yy, XX, YY = draw_polynet(surf, surface2, polyhedron, startface, startorientation, p1, p2, tilingname,
                                      polyname, unusedfaces)
        ...
        if (value != None and value["type"] == "roller"):
            ...
            for group_index, group_data in enumerate(value["all_data"]):
                if group_data and group_data["type"] == "roller":
                    ...
                    used_faces = set(f for cfo_group in value["CFO_class_groups"][group_index] for c, f, o in
                                     value["CFO_classes"][cfo_group])
                    unused_faces = existing_faces - used_faces

                    face_counter = {facesize: sum(1 for face in net if len(net[face]) == facesize) for facesize in
                                    set(len(net[face]) for face in used_faces)}
                    ...
                    all_cfo = {(c, f, o) for cfo_group in value["CFO_class_groups"][group_index] for c, f, o in
                               value["CFO_classes"][cfo_group]}
                    ...
                    try:
                        os.mkdir("rolled_nets/")
                    except:
                        pass
                    rollerscount += 1
                    filename = "rolled_nets/[%03d]" % rollerscount + tilingname + " @ " + polyname + gc + ".png"

                    draw_answer(filename, tilingname, polyname, None, mapping, net, p1, p2, f, o, area2[2], area2[3],
                                unused_faces, svg=False)
                    draw_answer(filename, tilingname, polyname, None, mapping, net, p1, p2, f, o, area2[2], area2[3],
                                unused_faces, svg=True)
