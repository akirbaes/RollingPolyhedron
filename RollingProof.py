...
def is_roller(tiling, tilingname, net, polyname):
    ...
    """Borders helper to know in which direction to roll"""
    borders = generate_supertile_coordinate_helpers(tiling, tilingname)
    # dict[border fo]=Union(coordinates shifts,other fo)
    # print(borders)
    """Classes transformations to know what are the related states"""
    ...
    """Turn class transformation into integers to make them easier to manipulate"""
    cts = set()
    ctsd = dict()
    ...
    """For every connected group, explore the transformations up to N supertiles away to build symmetry vectors"""
    """Then use those symmetry vectors to fill a minimal representation"""
    for groupindex, group in enumerate(groups):
            ...
            if (st, x, y) not in explored:
                ...
                for next_st, (dx, dy) in next:
                    if next_st == startingstate and (x + dx, y + dy) != (0, 0):
                        ...
                        if (len(min_symmetries) < 1 and [nx, ny] not in min_symmetries):
                        ...
                        elif ((len(min_symmetries) > 0 or sqrdist((nx, ny)) <= max(
                                sqrdist(sym) for sym in min_symmetries)) and [nx, ny] not in min_symmetries):
                            min_symmetries += [[nx, ny]]
                            # print(symmetries)
                            ...
                            while ((len(inds) < len(min_symmetries))):
                                ...
                                for size, index in reversed(
                                        sorted((sqrdist(sym), id) for id, sym in enumerate(min_symmetries))):
                ...
                if ([-1, 0] in min_symmetries or [1, 0] in min_symmetries) and (
                        [0, 1] in min_symmetries or [0, -1] in min_symmetries):
        ...
        def trim_points(points, vec1, vec2):
            ...
            for index, pt in enumerate(pointslist):
                symmetries = tuple(f(pt, v) for f in (ptAdd, ptSub) for v in (vec1, vec2)) + tuple(
                    f1(f2(pt, vec1), vec2) for f1 in (ptAdd, ptSub) for f2 in (ptAdd, ptSub))
        ...
        if is_roller:
            ...
            # print(tilingname, polyname, "%i/%i" % (groupindex + 1, len(groups)),
            #       "is not a roller in -%i:%i" % (N, N))
            if (is_quasi_roller):
                type = "quasi_roller"
                all_data[groupindex]["type"] = type
            else:
            ...
            if (N):
                graph = [[bool((i, j) in filled_supertiles) + bool(explored_states[(i, j)]) for i in
                          range(-N // 2, N // 2 + 1, 1) if (i, j) in explored_states] for j in
                         range(-N // 2, N // 2 + 1, 1)]
                # CFOClassGenerator.prettyprint_012(graph)
                # input()
        if (GENERATE_PROOF):
            if not any(planeroller[:groupindex]) or DUPLICATE_IMAGES:
                generate_image(tiling, net, tilingname, polyname, classes, group, groups, borders, min_symmetries,
                               explored_states, type)
    ...
    if True in planeroller:
        ...
        for explored_areas in exploreds:
            ...
            reached_states.append(
                set((c, f, o, x, y) for (c, f, o) in classes[st] for (st, x, y) in explored_areas))
        i = 0;
        merged_states = []
        merged_areas = []
        while reachable_areas:
            ...
            while ra in reachable_areas:
                ...
                rs = rs.union(reached_states.pop(index))
            merged_states.append(rs)
            merged_areas.append(ra)
        merged_states_per_tile = dict()
        for index, area in enumerate(merged_areas):
            for c, x, y in area:
                for _c, f, o, _x, _y in merged_states[index]:
...
if __name__ == "__main__":
    ...
    if (UPDATE_RESULTS):
        ...
        with open('_results/rollersdata.pickle', 'wb') as handle:
            pickle.dump((rollersdata, tuple(all_tilings.keys()), tuple(all_nets.keys())), handle,
